  ...
  switch (action.type) {
    ...
    case "FILTER_DATA":
      if ((action.category !== '' || action.textFilter !== '') && initialState.raw) {
        ...
        let filtered = raw?.filter((item: Item) => item.type === 'transaction') || [];

        if (action.category) {
          filtered = filtered.filter((item: Item) => item.cat === action.category);
        }
        ...
        const newState = filtered.reduce((accumulator: any, item: Item) => {
