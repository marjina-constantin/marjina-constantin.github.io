...
export default function SavingsHistory() {
  ...
  for (const item of dataInChronologicalOrder) {
    ...
    if (item.type === 'incomes') {
      totalIncomesAtDate = parseFloat(String(totalIncomesAtDate)) + parseFloat(item.sum);
    }
    else {
      totalExpensesAtDate = parseFloat(String(totalExpensesAtDate)) + parseFloat(item.sum);
    }

    savings[item.dt] = [
      itemDate.getTime(),
      parseFloat(parseFloat(String(((totalExpensesAtDate / totalIncomesAtDate) - 1) * -100)).toFixed(2)),
    ];
  }
